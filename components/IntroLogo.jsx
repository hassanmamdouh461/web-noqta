...
/**
 * IntroLogo component
 * Renders the center logo shown during the scribble transition.
 * Supports custom children, custom text, Noqta brand (default), or original Truus vector logo.
 */
export default function IntroLogo({
    ...
    if (children) {
        return (
            <div className={`intro-center-logo ${className}`} style={{ color }}>
                {children}
            </div>
        );
    }
    ...
    if (text) {
        return (
            <div className={`intro-center-logo ${className}`} style={{ color }}>
                <span style={{ fontSize: 'clamp(2rem, 6vw, 4.5rem)', fontWeight: 800, letterSpacing: '-0.03em' }}>
                    {text}
                </span>
            </div>
        );
    ...
    // Default: Noqta brand mark & Arabic calligraphy
    return (
        <div
            className={`intro-center-logo ${className}`}
            style={{
                color,
                display: 'flex',
