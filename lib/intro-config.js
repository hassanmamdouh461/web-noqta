...
export const DEFAULT_INTRO_CONFIG = {
    ...
    // Play the intro only on the first page view of a browser session.
    // The full animation lasts durationIn + delayBeforeOut + durationOut
    // (~4.7s) and sets `pointer-events: none` on <body> while it runs, so
    // replaying it on every reload / back-navigation locks the visitor out of
    // the site each time. Set to false to make it run on every load.
